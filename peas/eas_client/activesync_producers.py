...
class WBXMLProducer:
    def __init__(self, wbdoc, verbose=False):
        ...
        self.wb = wbdoc
        self.body = wbdoc.tobytes().decode("utf-8")
...
def convert_dict_to_wbxml(indict, default_page_num=None):
	...
	if default_page_num != None:
		root = wbxmlelement(list(indict.keys())[0], page_num=default_page_num)
	else:
		root = wbxmlelement(list(indict.keys())[0])
	wb.addchild(root)
	convert_array_to_children(root, list(indict.values())[0])
	return wb
