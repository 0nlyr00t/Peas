...
import base64, urllib.parse, io, uuid, sys
...
class WBXMLHandler(protocol.Protocol):
    ...
    def connectionLost(self, reason):
        ...
        if "Status" in list(res_dict.values())[0]:
            err_status = int(list(res_dict.values())[0]["Status"])
            if err_status != 1:
...
class ActiveSync(object):
    ...
    def add_parameters(self, url, params):
        ps = list(urllib.parse.urlparse(url))
        ps[4] = urlencode(params)
        return urllib.parse.urlunparse(ps)
    def authorization_header(self):
        return "Basic " + base64.b64encode(f"{self.domain.lower()}\\{self.username.lower()}:{self.password}".encode()).decode()
